...
echo "" >> changelog.txt && \
echo "" >> changelog.txt && \
echo "### :package: [Download JAR](https://oss.sonatype.org/service/local/repositories/releases/content/com/erudika/para-cache-hazelcast/${ver}/para-cache-hazelcast-${ver}-shaded.jar)" >> changelog.txt && \
hub release create -F changelog.txt $ver && \
rm changelog.txt

docker build -t erudikaltd/para-cache-hazelcast:${ver} . && docker push erudikaltd/para-cache-hazelcast:${ver}
